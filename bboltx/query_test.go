...
func TestListKeysValues(t *testing.T) {
	...
	entryList, err := users.EntryList(ctx, bboltx.WithPrefix[string]([]byte("a/")))
	if err != nil {
	...
	keyList, err := users.KeyList(ctx, bboltx.WithStart("a/2"), bboltx.WithEnd("b/1"))
	if err != nil {
	...
	valueList, err := users.ValueList(ctx, bboltx.WithPrefix[string]([]byte("a/")))
	if err != nil {
...
func TestIterAndWalk(t *testing.T) {
	...
	err = users.Walk(
		ctx,
		func(entry bboltx.Entry[string, user]) error {
